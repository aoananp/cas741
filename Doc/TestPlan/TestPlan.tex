...
software implementation.  \wss{I deleted the extra newline you inserted.  You
  ...
  manually inserting newlines.} \wpa{Noted - thank you.}
...
The following equation \wss{say equation, not algorithm} \wpa{Reworded.} will be implemented to
compare the results:
...
\wpa{The equation has been modified to take the norm of the vector instead. The testing software will also
have the ability to output the plot of the error percent of each step.}
...
\item{Static Analyzer\\} \wss{proof read} \wpa{Proof read.}
...
analysis of code updates.  \wss{How does the build server find Matlab?  This is
  ...
  something special to deal with the Matlab licensing.} \wpa{MATLAB 2017a
  has a built-in Build Server that seamlessly integrates with Git.}
...
is provided in the Appendix Section \ref{checklist}.
 ...
 \wss{You should be more specific about how you are
  ...
  your appendix.}
  \wpa{The code inspection checklist is provided in Section \ref{checklist}}
...
\wss{Great to see that you have additional details.  If I have
  any comments on the walkthrough, I'll make them when I review that section.}
  \wpa{The code inspection checklist is shown in Section \ref{checklist}.
  The walkthrough report will include the details of each specific iteration of the code
  and will have to based on the actual code itself. The Commonality Analysis and the Unit Test plan
  will serve as the heuristic guides in carrying out the code walkthrough.}
...
conditions are chosen.  \wss{Are you doing symbolic execution?  There isn't
  ...
  tool are you using?}
  \wpa{In the interest of time, details of symbolic execution testing and and its execution will be covered in
  Revision 2 of the document and the next iteration of the waterfall model.}
...
\wss{I would like more information in this table.  Why is the success false?  I
  ...
  semantic meaning.}
    \wpa{Clarifications to the table have been made.}
...
How test will be performed: Automated system test  \wss{For these test cases, it
  ...
  make it clear.}
  \wpa{The test will be performed as such. The listed inputs are for the tester to input to the program.
  The output will be verified when compared with the MATLAB solution.}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-euler_nonlinear}: Linear Trigonometric Case}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-euler_nonlineariterative}: Linear Trigonometric Iterative Case}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-trap_nonlinear}: Linear Trigonometric Case}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-trap_nonlineariterative}: Linear Trigonometric Iterative Case}}
...
\wss{It looks like you are repeating inputs for different tests.  I think this
  ...
  \wpa{Noted. This will be addressed in the next revision of the Test Plan and the next waterfall iteration.
  For now, the focus is on test execution - the document's content is valid for now for our purposes.}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-heun_nonlinear}: Linear Trigonometric Case}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-heun_nonlineariterative}: Linear Trigonometric Iterative Case}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-rk_nonlinear}: Linear Trigonometric Case}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{t-rk_nonlineariterative}: Linear Trigonometric Iterative Case}}
...
\wss{Are you just taking one step for each of these tests?  You really also need
 ...
 \wpa{These tests are the comparing the results of the ODE to the solutions worked out by hand.
 The testing method to be used is by relying on the MATLAB's solution as the oracle and taking the
 norm of the relative error vector - this is described early on in the document. The test report will
 indeed be a summary of observed errors. The outputs are
 left here for reference and for discussion in the Test Report.}
...
Output/Result: $\sigma_\text{\famname{}} \leq mult * \sigma_\text{MATLAB}$ (\famname{}' runtimes shall be no more 
than $mult$ times that of MATLAB's. For now, $mult = 4$ is defined.)  \wss{Use a symbolic constant here, so
  that it is easy to change.  Also, this requirement should be in the SRS.  Is
  it there?  If not, you should add it.} \wpa{This requirement is in the CA.}
...
\wss{Is this just one step of the ODE solver?  I don't think you'll be able to
  ...
  \wpa{This is through all the steps in the ODE solver, executing from $x_0$ to $x_k$ and $y_0$ to $y_k$.}
...
The intermediate $y$ values (Result) will be compared according to the following formula of the relative error norm -
$$\epsilon_{\text{relative}} = \frac{\text{Result}_\text{MATLAB} - \text{Result}_\text{\famname{}}} {\text{Result}
_\text{MATLAB}} $$
...
\wss{I don't think I see a test related to accuracy where you compare the
  ...
  \wpa{For testing, the MATLAB ODE solver will serve as the oracle. The comparisons will be made against it
  and observations against the desktop execution (outputs described in the Functional Tests) of the solution will be described.}
...
\wss{Are you implementing a parser?  I thought that Matlab was just taking care
  of this?}
  \wpa{MATLAB will take care of the parser - I figured since it was part of my requirement, that I shall test it as well.}
...
\item{Does the code have the correct inputs and outputs?}  \wss{What does this
    ...
    \wpa{It shall agree with the inputs and outputs listed in the functional requirements listed above through code walkthrough.}
...
\wss{This isn't actually a code walkthrough procedure.  This is a code
  ...
  \wpa{Noted - that was the intent of the section :-). The section has been renamed.}
