...
\item[\refstepcounter{acnum} \actheacnum \label{acLanguage}:] The specific language
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:] There will always be
  a source of input data external to the software such as a function call with arguments.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:] The format of the
  initial input data.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInputParams}:] The format
of the input parameters.
\item[\refstepcounter{ucnum} \uctheucnum \label{ucOutput}:] The format of the
initial output data.
...
Similarly, M\ref{mODEParse}, M\ref{mOutputFormat}, and M\ref{mPlot} are already available in Matlab and will not be reimplemented.
...
\multirow{3}{0.3\textwidth}{Software Decision Module} & {Equation String Parser Module}\\
& Plotting Module\\
