...
The scope of the family is limited to the the library of ODE solvers. Given
...
shall use \famname{} in solving ODE Initial Value Problems (IVPs).
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Label&\bf Initial Value Problem (IVP) of an Ordinary Differential Equation (ODE)\\
  \hline
  Equation& Implicit form of the $n^{th}$ order ODE:\\
  &$f(x, y, y',...y^\text{(n)}) = o$\\
  &Explicit form of the $n^{th}$ order ODE:\\ 
  ...
  \hline
  Ref.\ By & \ddref{D_SLOPE}, \tref{T_uniqsoln}, \iref{euler}, \iref{heun}\\
  \hline
\end{tabular}
\end{minipage}\\
