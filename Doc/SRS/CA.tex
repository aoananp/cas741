...
\item Convert the high order ODE to a system of first order ODEs
\item Perform the intermediate matrix calculations to solve the system of first order ODEs
\end{itemize}
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Ref.\ By & \tref{T_IVP}, \tref{T_HODE}, \iref{euler}, \iref{heun}\\
  \hline
\end{tabular}
\end{minipage}\\
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  \hline
  \rowcolor[gray]{0.9}
  Number& T\refstepcounter{theorynum}\thetheorynum \label{T_HODE}\\
  \hline
  Label&\bf Higher Order Ordinary Differential Equation\\
  \hline
  Equations& [\tref{T_ODE}]\\
    &$y^{(n)} = f(x,y,y^\text{(1)},y^\text{(2)},...,y^\text{(n-1)})$\\
    &$f_\text{(1)} = y^\text{(1)},, f_\text{(2)} = y^\text{(2)},..., f_\text{(n)} = y^\text{(n)}$\\
    & $f = $
    \[
        \left[
          \begin{tabular}{c}
  ...
  \hline
  Description & 
                The above theoretical model shows a higher order ODE represented as a system
                of first order ODE. This model allows for a more manageable approach to
                finding the solution to a higher order ODE.
                \\
  \hline
  Source &
           \url{http://www.math.poly.edu/courses/ma2132/Notes/MA2132EquationsToSystems.pdf}\\
  % The above web link should be replaced with a proper citation to a publication
  \hline
  Ref.\ By & \iref{euler}, \iref{heun}\\
  \hline
...
Method, Fourth-Order Taylor Series, Fourth-Order Runge Kutta) will be used in solving
the ODE.

\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_typeoff}:]
The types of $f(x,y)$ are linear, nonlinear, homogeneous, and non-homogeneous
equations.
